...
| **제안 배경** | Transformer 원본 구조 | • PostLayerNorm의 학습 불안정성 해결<br>• Warm-up 의존성 문제 개선 | • LayerNorm의 연산 비용 문제<br>• 평균/분산 계산으로 인한 병목 현상<br> • PreLayerNorm의 양상이 Tanh와 유사 |
...
| **주요 단점** | • 긴 Warm-up 필요<br>• 학습 초기 불안정<br>• 기울기 소실/폭주 위험 | •  | • 상대적으로 새로운 기법<br>• 검증 사례가 제한적 |
...
2. **전역 의존성**: LayerNorm에서는 각 값의 정규화가 전체 임베딩 차원 값들에 의존하므로, 스레드 간 데이터 공유와 동기화가 반드시 필요
