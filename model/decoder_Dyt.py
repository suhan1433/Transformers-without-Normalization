...
class DecoderBlock(nn.Module):
    def __init__(
        ...
        self.masked_dyt = DyT(hidden_size)

        # cross attention
        self.attn = MultiHead(hidden_size, n_splits)
        self.attn_norm = nn.LayerNorm(hidden_size)
        self.attn_dropout = nn.Dropout(dropout_p)
        self.attn_dyt = DyT(hidden_size)
    ...
    def forward(self, x, k_and_v, mask, prev, future_mask):
        ...
        normed_k_and_v = self.attn_dyt(k_and_v)
        
        z = z + self.attn_dropout(self.attn(Q = self.attn_dyt(z), K= normed_k_and_v, V= normed_k_and_v, mask=mask))
